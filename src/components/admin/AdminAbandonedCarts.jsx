  ...
  return (
    <div>
      ...
      {!orders ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
          ...
          <div className="card-soft p-4 bg-green-50">
            ...
            <p className="text-xs text-foreground/55 mt-0.5">قيمة محتملة ضائعة (ريال)</p>
          </div>
      ...
      {!orders ? (
        ...
        <div className="card-soft p-10 text-center text-foreground/50"><ShoppingCart className="w-10 h-10 mx-auto mb-3 opacity-40" />لا توجد سلات معلّقة حاليًا — 🎉</div>
      ) : (
        <>
          {abandoned.length > 0 && (
            <div className="space-y-3 mb-6">
              <h4 className="font-bold text-sm text-amber-600">متروكة فعليًا ({abandoned.length})</h4>
              {abandoned.map(o => (
                <div key={o.id} className="card-soft p-4 flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-foreground/50 truncate">{o.product_name} • {o.total} ريال • {o.city}</p>
          ...
          {active.length > 0 && (
            <div className="space-y-3">
              <h4 className="font-bold text-sm text-blue-600">قيد الإكمال الآن ({active.length}) <span className="text-foreground/40 font-normal">— أقل من {ACTIVE_WINDOW_MIN} دقيقة، ممكن لسه يكمل الدفع</span></h4>
              {active.map(o => (
                <div key={o.id} className="card-soft p-4 flex flex-wrap items-center justify-between gap-3 opacity-80">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center flex-shrink-0"><Clock className="w-5 h-5 text-blue-600" /></div>
                    <div className="min-w-0">
                      <p className="font-bold text-sm truncate">{o.customer_name} <span className="text-foreground/50 font-normal" dir="ltr">— {o.phone}</span></p>
                      <p className="text-xs text-foreground/50 truncate">{o.product_name} • {o.total} ريال • {o.city}</p>
                      <p className="text-[11px] text-blue-600 font-medium mt-0.5">{timeAgo(o.created_date)}</p>
                    </div>
                  </div>
                  <button onClick={() => remove(o.id)} className="p-2 rounded-lg text-destructive/60 hover:bg-red-50 flex-shrink-0"><Trash2 className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
          )}
